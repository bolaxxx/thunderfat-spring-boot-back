...
@RestController
@RequestMapping(value="/filtroAlimentario")
public class FiltroAlimentarioRestController {
@Autowired
private IFiltroAlimentarioService servicefiltro;

@GetMapping(value="/{id}/")
public List<FiltroAlimentario> listarporNutriconista(@PathVariable(name="id") int id ) {
	return servicefiltro.listarporNutricionista(id);
}
@GetMapping(value="/detalle/{id}/")
public FiltroAlimentario buscaPorId(@PathVariable(name="id") int id ) {
	
	return servicefiltro.buscarPorId(id);
}
@PostMapping(value="/save/{id}")
public FiltroAlimentario guarPaciente(@RequestBody FiltroAlimentario newPaciente,@PathVariable("id")int id_nutricionista) {
	System.out.println(newPaciente);
	servicefiltro.insertar(newPaciente,id_nutricionista);
	return null;
}
