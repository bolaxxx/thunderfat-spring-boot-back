...
public class ModernAuthorizationServerConfig {
    ...
    SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http)
            throws Exception {
        // Configure OAuth2 Authorization Server security
        http.securityMatcher("/oauth2/**", "/.well-known/**", "/login")
    ...
    RegisteredClientRepository registeredClientRepository() {
        RegisteredClient angularClient = RegisteredClient.withId(UUID.randomUUID().toString())
                .clientId("angularapp")
                .clientSecret("{noop}12345") // Use {noop} for plain text passwords in development
